...
struct ReadingSessionView: View {
            ...
            HStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if playbackManager.isLoading {
                        ...
                        Text(playbackManager.isPlaying ? "Playing" : "Paused")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
    ...
    func startAdaptiveMusic() async {
        if let selectedGenres = UserDefaults.standard.array(forKey: "selectedGenres") as? [String],
           let firstGenre = selectedGenres.first {
            await playbackManager.startAdaptiveReadingMusic(spotifyManager: spotifyManager, genre: firstGenre)
        } else {
            // Default to classical instrumental if no genre selected
            await playbackManager.startAdaptiveReadingMusic(spotifyManager: spotifyManager, genre: "classical")
        }
    }
}
